        ...
        <div class="card-header d-flex justify-content-between align-items-center py-3 border-bottom rounded-top-4" style="background-color: #13171d; border-color: #21262d !important;">
            ...
            <div>
                @if($pesanan->status === 'completed')
                    <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-2 rounded-pill fw-medium"><i class="bi bi-check2-all me-1"></i> Selesai</span>
                @else
                    <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-3 py-2 rounded-pill fw-medium"><i class="bi bi-x-circle me-1"></i> Dibatalkan</span>
                @endif
            </div>
        </div>
        <div class="card-body" style="background-color: #0e1217;">
            ...
            @if($pesanan->discount_amount > 0)
                <h6 class="fw-bold text-white mt-2 mb-0">Total ({{ $pesanan->detail_pesanan->sum('jumlah') }} Item): <span class="text-secondary text-decoration-line-through fw-normal small">Rp {{ number_format($pesanan->total, 0, ',', '.') }}</span> <span style="color: #c08e5c;">Rp {{ number_format($pesanan->total - $pesanan->discount_amount, 0, ',', '.') }}</span></h6>
            @else
                <h6 class="fw-bold text-white mt-2 mb-0">Total ({{ $pesanan->detail_pesanan->sum('jumlah') }} Item): <span style="color: #c08e5c;">Rp {{ number_format($pesanan->total, 0, ',', '.') }}</span></h6>
            @endif
        </div>
            ...
            <div class="card-footer py-3 rounded-bottom-4" style="background-color: #13171d; border-top: 1px solid #21262d !important;">
                @if(!$pesanan->rating)
                    <form action="/konsumen/rating/store" method="POST" class="p-3 rounded-3" style="background-color: rgba(192, 142, 92, 0.06); border: 1px solid rgba(192, 142, 92, 0.2);">
                        ...
                        <label class="small fw-bold mb-2 d-block" style="color: #c08e5c;"><i class="bi bi-star-fill text-warning me-1"></i> Berikan Penilaian untuk Pesanan Ini</label>
                        <div class="row g-2">
                            <div class="col-md-4">
                                <select name="rating" class="form-select text-white" style="background-color: #0e1217; border-color: #21262d;" required>
                            ...
                            <div class="col-md-6">
                                <input type="text" name="komentar" class="form-control text-white" style="background-color: #0e1217; border-color: #21262d;" placeholder="Ulasan Anda (Opsional)">
                            </div>
                            <div class="col-md-2">
                                <button type="submit" class="btn btn-touch w-100 fw-bold" style="background: var(--gradient-bronze); color: white; border: none;">Kirim</button>
                            </div>
                        </div>
                    </form>
                @else
                    <div class="d-flex justify-content-between align-items-center p-3 rounded-3" style="background-color: #0e1217; border: 1px solid #21262d;">
                        <div>
...
@empty
    <div class="text-center py-5 px-3">
        ...
        <h5 class="fw-bold text-white mb-2" style="font-size: 1.2rem; letter-spacing: -0.01em;">Belum Ada Riwayat</h5>
        <p class="text-secondary small mx-auto mb-4" style="max-width: 320px; line-height: 1.55;">
            Riwayat pesanan Anda yang sudah selesai akan tampil di sini.
        </p>
        <a href="{{ url('/katalog') }}" class="btn btn-touch px-4 py-2 rounded-pill fw-semibold shadow-sm d-inline-flex align-items-center gap-2" style="background: var(--gradient-bronze); color: #ffffff; border: none; font-size: 0.9rem;">
