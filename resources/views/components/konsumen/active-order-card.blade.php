    ...
    <div class="card border-0 shadow-sm mb-4 rounded-4" style="background-color: #0e1217; border: 1px solid #21262d !important; overflow: hidden;">
        <div class="card-header d-flex justify-content-between align-items-center py-3 border-bottom rounded-top-4" style="background-color: #13171d; border-color: #21262d !important;">
            ...
            <div>
                @if($pesanan->status === 'pending')
                    <span class="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 px-3 py-2 rounded-pill fw-medium"><i class="bi bi-hourglass-split me-1"></i> Menunggu Diproses</span>
                @elseif($pesanan->status === 'processing')
                    <span class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 px-3 py-2 rounded-pill fw-medium"><i class="bi bi-fire me-1"></i> Sedang Dimasak</span>
                @elseif($pesanan->status === 'completed')
                    <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-2 rounded-pill fw-medium"><i class="bi bi-check2-all me-1"></i> Selesai Dimasak</span>
                @endif
            </div>
        </div>
        <div class="card-body" style="background-color: #0e1217;">
            <div class="d-flex align-items-center mb-3">
                ...
                <div>
                    <h6 class="mb-0 fw-bold text-white">{{ $pesanan->tipe_pesanan == 'takeaway' ? 'Bungkus / Takeaway' : 'Makan di Tempat' }}</h6>
            ...
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="text-end">
                    ...
                    @if($pesanan->discount_amount > 0)
                        ...
                        <h5 class="fw-bold mb-0" style="color: #c08e5c;">Rp {{ number_format($pesanan->total - $pesanan->discount_amount, 0, ',', '.') }}</h5>
                    @else
                        <h5 class="fw-bold mb-0" style="color: #c08e5c;">Rp {{ number_format($pesanan->total, 0, ',', '.') }}</h5>
                    @endif
                </div>
            </div>
            @if($hasAction)
                <div class="d-flex justify-content-end gap-2 flex-wrap border-top pt-3 mt-3" style="border-color: #21262d !important;">
                    @if($pesanan->pembayaran->status == 'unpaid')
                        <a href="/konsumen/checkout/{{ $pesanan->id }}" class="btn fw-semibold px-4 rounded-pill btn-touch" style="background: var(--gradient-bronze); color: white; border: none; font-size: 0.875rem;">
                            Pilih Pembayaran <i class="bi bi-arrow-right ms-1"></i>
                        </a>
                    @endif
                    @if($pesanan->tipe_pesanan === 'dine_in' && $pesanan->id_meja)
                        <button onclick="callBell({{ $pesanan->id_meja }})" class="btn btn-outline-danger fw-semibold px-4 rounded-pill btn-touch" style="font-size: 0.875rem;">
...
@empty
    <div class="text-center py-5 px-3">
        ...
        <h5 class="fw-bold text-white mb-2" style="font-size: 1.2rem; letter-spacing: -0.01em;">Belum Ada Pesanan Aktif</h5>
